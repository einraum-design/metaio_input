...
public:
	...
	*
	* This function must be called within the OpenGL thread. It should be only called once
	*
	...
	*/
	virtual void initializeRenderer(int width, int height, ESCREEN_ROTATION screenRotation=ESCREEN_ROTATION_0, 
		HWND windowHandle = 0, const ERENDER_SYSTEM renderSystem = ERENDER_SYSTEM_OPENGL) = 0;
...
METAIO_DLL_API IMetaioSDKWin32* CreateMetaioSDKWin32(int width, int height,
        ESCREEN_ROTATION screenRotation=ESCREEN_ROTATION_0, HWND windowHandle = 0,
        const ERENDER_SYSTEM renderSystem = ERENDER_SYSTEM_OPENGL_ES_2_0);
